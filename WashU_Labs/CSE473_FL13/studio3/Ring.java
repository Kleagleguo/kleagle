...
public class Ring {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (now - ringTestTime >= tenSec) {
				if (!gotRingReply) System.out.println("ring break detected");
				testPkt.send(sock, succAdr, debug);
				gotRingReply = false;
			}
		}
	...
	public static void join(InetSocketAddress predAdr) {
		...
		join.type = "join";
		join.send(sock, predAdr, debug);
		Packet reply = new Packet();
		InetSocketAddress addr = reply.receive(sock, debug);
		if (addr == predAdr && reply.type.equals("accept")) {
			succAdr = new InetSocketAddress(reply.succIp, reply.succPort);
		}
	}
	...
	public static void handleJoin(Packet p, InetSocketAddress joinerAdr) {
		// TODO
		Packed hj = new Packet();
		hj.type = "accept";
		hj.succIp = succAdr.getAddress();
		hj.succPort = succAdr.getPort();
		reply.send(sock, joinerAdr, debug);
	}
