	...
	public void run() {
		...
		while (!quit) {
			if (sub.incoming()) {
				...
				Packet p = null;
				p = sub.receive();
				String str = p.payload;
				pktsRcvd++;
				charsRcvd += str.length();
				pktsReported = p.pktCount;
				charsReported = p.charCount;
				toSnk.offer(str);
			} else if (fromSrc.size() > 0 && sub.ready()) {
				// there is a message to send to substrate
				// TODO - form next packet and send to substrate
				Packet p = new Packet();
				String msgToSend = fromSrc.poll();
				pktsSent++;
				charsSent += msgToSend.length();
		...
		// print a report
		System.out.println("pkts Sent = " + pktsSent + " pkts Rcvd = " + pktsRcvd);
		System.out.println("chars Sent = " + charsSent + " chars Rcvd = " + charsRcvd);
		System.out.println("pkts Reported = " + pktsReported);
		System.out.println("chars Reported = " + charsReported);
	}
