...
public class Forwarder implements Runnable {
	...
	public void run() {
		...
		while (!quit) {
			...
			if(sub.incoming()) {
				Pair<Packet, Integer> incoming = sub.receive();
				if(incoming.left.destAdr == myIp) {
					try {
					toSnk.put(incoming.left);
					} catch (Exception e) {
					...
					if(p.ttl == 0) {
						if(debug>0) {
							System.out.println("Packet " + p + " has been discarded");
						}
					} else {
						int lnk = incoming.right;
						int rand_lnk = r.nextInt(3);
						while(rand_lnk == lnk) {
							rand_lnk = r.nextInt(3);
						}
						sub.send(p, rand_lnk);
					}
				}
			} else if(fromSrc.peek() != null) {
				try {
					p = fromSrc.take();
					int rand_lnk = r.nextInt(3);
					sub.send(p, rand_lnk);
